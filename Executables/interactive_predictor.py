...
def main():
    ...
    print("\n" + "="*60)
    print("VALORANT MATCH PREDICTOR - INTERACTIVE MODE")
    print("="*60)
    ...
    available_maps = ["Split", "Icebox", "Ascent", "Haven", "Fracture", "Lotus", "Pearl"]
    ...
    while True:
        ...
        # Get user input
        team1 = input("Team 1 name: ").strip()
        if team1.lower() == 'quit':
            break
            
        team2 = input("Team 2 name: ").strip()
        if team2.lower() == 'quit':
            break
            
        map_name = input("Map name: ").strip()
        if map_name.lower() == 'quit':
            break
        
        # Validate inputs
        if team1 not in available_teams:
            print(f"Error: '{team1}' is not a valid team name.")
            continue
            
        if team2 not in available_teams:
            print(f"Error: '{team2}' is not a valid team name.")
            continue
            
        if map_name not in available_maps:
            print(f"Error: '{map_name}' is not a valid map name.")
            continue
        ...
        try:
            ...
            print("Please check that all inputs are correct.")
